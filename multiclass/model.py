...
class MGF_DDI(nn.Module):
    def __init__(self, d_atom=37, d_edge_in=6, d_hidden=256, d_edge_hid=64, d_unimol=1536, rel_total=86,
        ...
        self.macro_encoder = MacroEncoder(
            ...
            d_unimol=d_unimol,
            gt_layers=gt_layers  # [修改] 传入参数，不再硬编码为 3
        )
        ...
        # 4. 融合
        # 注意：这里原本代码有重复定义 self.fusion，这里保留 TriViewFusion 作为最终逻辑
        # self.fusion = DynamicFusion(d_hidden * 2) # 被覆盖了，注释掉
    ...
    def forward(self, h_macro, t_macro, h_motif, t_motif):
        ...
        # 注意：这里如果不使用 DynamicFusion，v_inter 可能需要调整维度或者处理方式
