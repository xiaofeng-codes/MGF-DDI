...
def load_data(args, batch_size, fold_i):
    ...
    def data_unzip(data,dataset_name):
        if dataset_name == 'mdjcl':
            #D1,S1,D2,S2,label
            data_tup = [(h, t, r) for h, t, r in
                        zip(data['D1'], data['D2'], data['label'])]
            data_set = []
            for h, t, r in data_tup:
                data_set.append((h, t, r))
                data_set.append((t, h, r))

            return data_set
        else:

            data_tup = [(h, t, r, n) for h, t, r, n in
                        zip(data['Drug1_ID'], data['Drug2_ID'], data['Y'], data['Neg samples'])]
            data_set = []
    ...
    train_tri = pd.read_csv(f'{dataset_dir}/train_fold{fold_i}.csv', dtype=str)
    # val_tri = pd.read_csv(f'{dataset_dir}/test_fold{fold_i}.csv', dtype=str)
    test_tri = pd.read_csv(f'{dataset_dir}/test_fold{fold_i}.csv', dtype=str)

    train_tri = data_unzip(train_tri, args.dataset)
    train_tri, val_tri = split_train_valid(train_tri)
    # val_tri = data_unzip(val_tri, args.dataset)
    test_tri = data_unzip(test_tri, args.dataset)
